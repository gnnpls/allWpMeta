...
function sbp_spit_meta($view, $arrs, $id = 0, $extra = array(), $u = 0, $d = 0, $b = 0, $specific = '', $sep = ', ')
{
    /*
                ...
                switch ($d) {
                    case 0:
                        ...
                        switch ($a['case']) {
                            case 'message':
                                if (isset($a['value']) && !empty($a['value'])) {
                                    $ins = '<div class="sbp-meta-message" id="'.$original_meta_id.'"><div class="sbp-meta-message-label">'.$a['label'].'</div><div class="sbp-meta-message-inner">'.$a['value'].'</div></div>';
                                }
                                break;
                            case 'button':
                            ...
                            case 'section':
                                ...
                                foreach ($a['include'] as $key => $data) {
                                    ...
                                    $ins .= sbp_spit_meta(0, array($inputname => $data));
                                }
                                $ins .= '</div></div>';
                                ...
                                if (isset($a['sbp_tabs']) && !empty($a['sbp_tabs'])) {
                                    ...
                                    foreach ($a['sbp_tabs'] as $tab_id => $tab_intro) {
                                        ...
                                        foreach ($tab_intro['include'] as $key => $data) {
                                            ...
                                            $tab_contents .= sbp_spit_meta(0, array($inputname => $data));
                                        }
                                        $tab_contents .= '</div>';
                                    }
                        ...
                        if ($b != 4 && $b != 5 && !(isset($a['attributes']['exclude_meta']))) {
                            $ins .= '<input type="hidden" name="sbp_custom_meta[]" value="'.$original_meta.'"/>';
                        }
...
function sbp_save_custom_meta($data, $dataa, $id, $u = 0, $tt = '')
{
    if (isset($data) && !empty($data)) {
        $arr = sbp_custom_meta_update_vars($data, $dataa, $id, $u);
        do_action('sbp_custom_meta_update_action', $data, $dataa, $id, $u, $tt);

        return $arr;
    }
}

function sbp_custom_meta_update_vars($meta, $metaa, $id, $u)
{
    foreach ($meta as $k) {
        $chk = '';
